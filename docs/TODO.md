...
- [x] ~~The Transparent switch writes `TRANSPARENT` but never `ALPHA_MODE`~~ — **premise was wrong, and the real bug was next door.** `handleTransparentChange` already re-derived and wrote `ALPHA_MODE`; `handleAlphaTestChange` did not, and `alphaMode` is the only alpha field the shader reads (`MaterialTransmission` takes the opaque fast path on 0 and gates MASK on 1; `LightsDirect` matches). So the **Alpha Test slider was inert** — measured on AlphaBlendModeTest, setting `alphaTest` 0.5 on `MatOpaque` gave a bit-identical image (rmse 0.000000); writing the derived `alphaMode` too produces the cutout (rmse 0.031). Fixed in `da0c392e` by lifting the rule to one exported `deriveAlphaMode()` (it had three copies, two of which hardcoded `1023` for `RGBAFormat`) and routing the loader plus all three handlers through it. Still open, and genuinely harmless: a glTF that sets `alphaMode: BLEND` explicitly while `transparent` is false (the Gelatinous Cube) — BLEND always sets `alphaCutout`, so the shadow path's `transparent` branch is unreachable and the two cannot actually disagree in effect.
