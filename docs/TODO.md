...
- [ ] The Transparent switch writes `TRANSPARENT` but never `ALPHA_MODE`, so a material can sit in BLEND mode with `transparent == 0` (the Gelatinous Cube ships that way). Harmless in everything measured so far — BLEND's else-branch does not set `handled`, so the transmission branch still runs — but the two fields should not be able to disagree.
- [ ] `sceneFeatures.hasTransmission` goes stale after a UI transmission change (reads false while a material has `transmission == 1`). Cosmetic today: `SceneProcessor` only stores `sceneFeatures` and no shader path gates on it, so fix it or drop the field rather than leaving a lying flag.
