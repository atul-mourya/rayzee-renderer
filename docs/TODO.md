...
### Deferred
- ~~Reconsider continuous viewport denoising.~~ **Done.** The ≤15 ms bar was the wrong test — it is resolution-dependent and assumes per-frame denoising. Measured: a `fast` denoise costs a *resolution-invariant* **4.4 path-traced samples** (256²/512²/1024² = 8/23/89 ms against samples of 1.9/5.1/18.1 ms), so the gate is `4.4 / (samples between denoises)`. Shipped as a cadence floored at 2x the last denoise's own measured cost (the sample-growth gate was removed once the denoise got cheap — it only cost refreshes), off by default, pinned off in deterministic mode.
...
- ~~No app-side toggle for the cadence.~~ **Done.** OIDN is now an entry in the `Real-Time Denoiser` list (None / EdgeAware / ASVGF / NRD / OIDN), so only ever one thing denoises the live view — two would mean paying for a per-frame denoise the OIDN overlay then covers (measured waste: +2% NRD, +4% ASVGF, +35% EdgeAware). The `AI Denoising (OIDN)` switch keeps its single meaning: denoise the finished image. Two earlier shapes were tried and rejected — a three-way OIDN dropdown (read as a second real-time denoiser) and a nested `Update While Rendering` switch (still allowed both at once). `oidnQuality` now means the quality of the *finished* image; refreshes run the cheapest model with the same aux kind, swapping only on renders long enough to pay for the ~1 s weight reload.
